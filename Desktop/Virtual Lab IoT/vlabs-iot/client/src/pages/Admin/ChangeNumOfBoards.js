...
const getCurrentNumOfBoards = async (values) => {
  try {
    ...
    const response = await axios.get("http://localhost:4000/api/admin/get-num-of-boards", 
    {
      headers: {
  ...
  const onFinish = async (values) => {
    try {
      dispatch(showLoading());
      const response = await axios.post("http://localhost:4000/api/admin/change-num-of-boards", 
      {
        ...values, 
