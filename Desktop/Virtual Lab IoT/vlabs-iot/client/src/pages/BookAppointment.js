...
function BookAppointment() {
  ...
  function getDisabledHours() {
    var hours = [];
    for (let i = parseInt(moment().startOf('day').format("HH"),10);i < moment(experiment.timings[0]).format("HH"); i++) {
      hours.push(i);
    }
    for (let i = parseInt(moment(experiment.timings[1]).add(1, "hours").format("HH"),10);i <= parseInt(moment().endOf('day').format("HH"),10); i++) {
      hours.push(i);
    }
    // console.log(hours);
    return hours;
  ...
  const getExperimentData = async () => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:4000/api/user/get-experiment-info-by-id",
        {
  ...
  const checkAvailability = async () => {
    try {
      dispatch(showLoading());
      const response = await axios.post(
        "http://localhost:4000/api/user/check-booking-avilability",
        {
          experimentId : params.experimentId,
          date: date,
          time: time,
